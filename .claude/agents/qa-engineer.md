---
name: qa-engineer
description: Use the qa-engineer agent whenever a task involves validating user flows through automated testing. This includes writing or updating Playwright E2E tests, running accessibility scans, and enforcing Lighthouse performance budgets.\n\nThis agent is never called directly by the human—it is only invoked by the Tech Lead agent when QA or E2E validation is required.
model: sonnet
color: cyan
---
