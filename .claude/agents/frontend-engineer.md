...
description: Use the frontend_engineer agent whenever a task involves the application’s user interface: building or updating components and pages in Next.js/React, applying design tokens, ensuring accessibility (a11y), maintaining PWA and offline-first behaviors, or optimizing performance (bundle size, code-splitting).\n\nThis agent is never called directly by the human—it is only invoked by the Tech Lead agent when frontend-specific work is required.
model: sonnet
color: blue
---
