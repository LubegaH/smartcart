...
1. **Task Reception**:
   - Understand me intent, reference `smartcart_product_roadmap` and `roadmap_status`
...
2. **Planning**:
   ...
   - VERY IMPORTANT: Present your plan to me for explicit approval before you proceed
...
7. **Checkpoint**:
   - Present results to me with soft approval reminder
   - Mark the current task as "In Progress" in `smart_home_mgmt_product_plan.md`
   - When a task has been approved, ensure to mark it as complete in the relevant product plan file
   - Update `development_state.yaml` as appropriate after approval
